...
const consent = `
    ...
    <script is:inline>
      (() => {
        ...
        const dict={tr:['Çerez tercihleri','Analitik çerezleri yalnızca izninizle kullanıyoruz. Zorunlu çerezler her zaman aktiftir.','Reddet','Kabul et'],en:['Cookie preferences','We use analytics cookies only with your permission. Essential cookies are always active.','Reject','Accept'],fa:['تنظیمات کوکی','کوکی‌های تحلیلی فقط با اجازه شما استفاده می‌شوند. کوکی‌های ضروری همیشه فعال هستند.','رد','پذیرش'],mk:['Поставки за колачиња','Аналитички колачиња користиме само со ваша дозвола. Неопходните се секогаш активни.','Одбиј','Прифати'],sr:['Podešavanja kolačića','Analitičke kolačiće koristimo samo uz vašu dozvolu. Neophodni su uvek aktivni.','Odbij','Prihvati'],sq:['Preferencat e kukive','Kukit analitike përdoren vetëm me lejen tuaj. Kukit e domosdoshme janë gjithmonë aktive.','Refuzo','Prano']};
        const t=dict[document.documentElement.lang]||dict.en;
        box.querySelector('[data-cookie-title]').textContent=t[0];box.querySelector('[data-cookie-copy]').textContent=t[1];box.querySelector('[data-cookie-reject]').textContent=t[2];box.querySelector('[data-cookie-accept]').textContent=t[3];
        ...
        box.querySelector('[data-cookie-accept]').addEventListener('click',()=>apply('accepted'));box.querySelector('[data-cookie-reject]').addEventListener('click',()=>apply('rejected'));
      })();
    </script>
    <style is:global>
      .cookie-consent{position:fixed;z-index:9999;left:1rem;right:1rem;bottom:1rem;max-width:980px;margin:auto;padding:1rem 1.1rem;display:flex;align-items:center;justify-content:space-between;gap:1rem;border:1px solid #3a506b;border-radius:16px;background:rgba(7,13,22,.97);color:#f4f1e9;box-shadow:0 20px 60px rgba(0,0,0,.45)}.cookie-consent[hidden]{display:none}.cookie-consent p{margin:.25rem 0;color:#aab7c9;font-size:.9rem}.cookie-consent__actions{display:flex;gap:.6rem;flex:0 0 auto}.cookie-consent button{min-height:44px;padding:.65rem 1rem;border:1px solid #3a506b;border-radius:999px;background:#101c2b;color:#f4f1e9;font-weight:800;cursor:pointer}.cookie-consent [data-cookie-accept]{background:#f2aa8f;color:#09111d;border-color:#f2aa8f}@media(max-width:700px){.cookie-consent{align-items:stretch;flex-direction:column}.cookie-consent__actions{width:100%}.cookie-consent__actions button{flex:1}}
    </style>`;
