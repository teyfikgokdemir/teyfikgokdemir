...
export const posts: BlogPost[] = [
  ...
  },
  {
    ...
    sections: Object.fromEntries(blogLocales.map((l) => [l, [
      ...
      { heading: l==='tr'?'Sadece fiyata göre sıralamayın':l==='fa'?'فقط بر اساس قیمت رتبه‌بندی نکنید':l==='mk'?'Не рангирајте само по цена':l==='sr'?'Ne rangirajte samo po ceni':l==='sq'?'Mos renditni vetëm sipas çmimit':'Do not rank by price alone', paragraphs: [l==='tr'?'Ağırlıklı puanlama kullanın: teknik uygunluk, toplam maliyet, kalite kanıtı, kapasite, teslimat, ticari koşullar ve risk.':l==='fa'?'امتیازدهی وزنی به‌کار ببرید: انطباق فنی، هزینه کل، شواهد کیفیت، ظرفیت، تحویل، شرایط تجاری و ریسک.':l==='mk'?'Користете пондерирано оценување за техника, вкупен трошок, квалитет, капацитет, испорака, услови и ризик.':l==='sr'?'Koristite ponderisano ocenjivanje za tehniku, ukupan trošak, kvalitet, kapacitet, isporuku, uslove i rizik.':l==='sq'?'Përdorni vlerësim të ponderuar për teknikën, koston totale, cilësinë, kapacitetin, dorëzimin, kushtet dhe rrezikun.':'Use weighted scoring across technical fit, total cost, quality evidence, capacity, delivery, commercial terms and risk.'] },
      { heading: l==='tr'?'RFQ, müzakerenin başlangıcıdır':l==='fa'?'RFQ آغاز مذاکره است':l==='mk'?'RFQ е почеток на преговорите':l==='sr'?'RFQ je početak pregovora':l==='sq'?'RFQ është fillimi i negociimit':'The RFQ starts negotiation', paragraphs: [l==='tr'?'İlk teklif nihai sonuç değildir. Sapmaları ve varsayımları netleştirin, kısa liste oluşturun, teknik görüşme ve numune sonrasında ticari turu yeniden açın.':l==='fa'?'پیشنهاد اول نتیجه نهایی نیست. انحراف‌ها و فرض‌ها را روشن، فهرست کوتاه تهیه و پس از بررسی فنی و نمونه، مذاکره تجاری را باز کنید.':l==='mk'?'Првата понуда не е финална. Разјаснете ги отстапувањата, направете кратка листа и повторно отворете ги условите по техничката проверка.':l==='sr'?'Prva ponuda nije konačna. Razjasnite odstupanja, napravite uži izbor i ponovo otvorite uslove posle tehničke provere.':l==='sq'?'Oferta e parë nuk është përfundimtare. Sqaroni devijimet, bëni listën e shkurtër dhe rihapni kushtet pas verifikimit teknik.':'The first quote is not the final result. Clarify deviations, shortlist suppliers, complete technical review and samples, then reopen the commercial round.'] },
    ]])) as Record<BlogLocale, BlogSection[]>,
  },
];
