...
export default function GrowthIntelligencePanel({projectId}:{projectId:string|null}){
  ...
  const overviewMetrics=overview?.metrics30d;
  const hasOverviewMetrics=Boolean(overviewMetrics&&[overviewMetrics.spend,overviewMetrics.revenue,overviewMetrics.roas].some(value=>value!=null));
