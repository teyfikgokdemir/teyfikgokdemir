...
const selectStyle={minWidth:110,border:'1px solid rgba(183,148,246,.38)',background:'#171020',color:'#f6f1ff',borderRadius:12,padding:'10px 12px',font:'inherit',fontSize:13,fontWeight:650,outline:'none',cursor:'pointer',boxShadow:'inset 0 1px rgba(255,255,255,.03)'} as const;
...
export default function SearchConsolePanel({projectId}:{projectId:string|null}){
  ...
  function openAds(){
    const target=document.querySelector<HTMLButtonElement>('aside.side nav button[data-growth-section="ads"]')
      ||Array.from(document.querySelectorAll<HTMLButtonElement>('aside.side nav button')).find(button=>button.querySelector('span')?.textContent?.trim()==='Ads');
    target?.click();
  }
  ...
  const googleSetupNeeded=/google bağlantısı bulunamadı|google bağlantısı gerekli|google hesabı bağlı değil/i.test(error);

  if(!projectId)return <div className="empty">Önce bir proje seç.</div>;
  return <div className="moduleStack">
    <section className="moduleCard">
      <div className="reportHead compact"><div><p className="eyebrow">SEO / Search Performance</p><h2>Google Search Console</h2></div><select style={selectStyle} value={days} onChange={e=>setDays(Number(e.target.value))} disabled={loading||saving}><option value={7}>7 gün</option><option value={28}>28 gün</option><option value={60}>60 gün</option><option value={90}>90 gün</option></select></div>
      ...
      {googleSetupNeeded&&<div className="empty"><b>Search Console henüz bağlı değil.</b><div style={{marginTop:8,opacity:.75}}>Google hesabını bağladıktan sonra Search Console property’sini bu ekranda eşleştirip organik performansı izleyebilirsin.</div><button className="primaryAction" style={{marginTop:14,width:'auto'}} onClick={openAds}>Google’ı Bağla</button></div>}
      {!googleSetupNeeded&&error&&<div className="error">{error}</div>}
      {data?.matched&&<><div className="moduleFoot">{data.siteUrl} · {data.permissionLevel||'erişim mevcut'}</div>{canOverride&&<div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap',marginTop:14}}><select style={selectStyle} value={selectedSite} onChange={e=>setSelectedSite(e.target.value)} disabled={saving}><option value="">Property seç</option>{sites.map(site=><option key={site.siteUrl} value={site.siteUrl}>{site.siteUrl} · {site.permissionLevel||'erişim mevcut'}</option>)}</select><button className="primaryAction" onClick={selectSite} disabled={!selectedSite||saving||selectedSite===data.siteUrl}>{saving?'Kaydediliyor…':'Property Değiştir'}</button><span style={{opacity:.7,fontSize:13}}>Yanlış property eşleştiyse aktif proje için değiştirebilirsin.</span></div>}</>}
