...
export default function CommercePage(){
  ...
  return <main style={{minHeight:'100vh',padding:'40px',background:'#0b0712',color:'#f6f0ff'}}><div style={{maxWidth:1500,margin:'0 auto',display:'grid',gap:24}}><section className="moduleCard"><div className="reportHead compact"><div><p className="eyebrow">Growth OS · Commerce</p><h2>Merchant Center sağlık doğrulama</h2></div><select value={projectId} onChange={e=>setProjectId(e.target.value)}>{projects.map(p=><option key={p.id} value={p.id}>{p.name} · {p.domain}</option>)}</select></div></section><MerchantPanel projectId={projectId||null}/></div></main>;
}
