...
export default function AgencySettingsPage(){
  ...
  const canManage=Boolean(data&&['owner','admin'].includes(data.actor.role));

  async function save(event:FormEvent){
    event.preventDefault();
    if(!data?.actor.workspaceId||!canManage||saving)return;
    ...
    <div className="settingsShell">
      ...
      {!loading&&data?<div className="settingsGrid">
        <form className="settingsPanel" onSubmit={save}>
          ...
          <label><span>Marka adı</span><input value={brandName} onChange={e=>setBrandName(e.target.value)} maxLength={100} placeholder="Ajans adı" disabled={formDisabled}/></label>
          <label><span>Logo URL</span><input value={logoUrl} onChange={e=>setLogoUrl(e.target.value)} placeholder="https://..." disabled={formDisabled}/></label>

          <div className="colorGrid">
            <label><span>Ana renk</span><div className="colorInput"><input type="color" value={primaryColor} onChange={e=>setPrimaryColor(e.target.value)} disabled={formDisabled}/><input value={primaryColor} onChange={e=>setPrimaryColor(e.target.value)} pattern="#[0-9A-Fa-f]{6}" disabled={formDisabled}/></div></label>
            <label><span>Vurgu rengi</span><div className="colorInput"><input type="color" value={accentColor} onChange={e=>setAccentColor(e.target.value)} disabled={formDisabled}/><input value={accentColor} onChange={e=>setAccentColor(e.target.value)} pattern="#[0-9A-Fa-f]{6}" disabled={formDisabled}/></div></label>
          </div>

          <label><span>Özel domain</span><input value={customDomain} onChange={e=>setCustomDomain(e.target.value)} placeholder="portal.ajansiniz.com" disabled={formDisabled}/><small className="hint">DNS bağlama ayrı bir deployment adımıdır; bu alan şimdilik white-label domain kaydıdır.</small></label>
          <label><span>Rapor alt bilgisi</span><textarea value={reportFooter} onChange={e=>setReportFooter(e.target.value)} maxLength={500} rows={4} placeholder="Ajans adı · Müşteri raporu" disabled={formDisabled}/></label>

          <button className="saveButton" disabled={formDisabled}>{saving?'Kaydediliyor…':'Marka Ayarlarını Kaydet'}</button>
          {!canManage?<small className="hint">Bu alanı değiştirmek için admin veya owner rolü gerekir.</small>:null}
        </form>
