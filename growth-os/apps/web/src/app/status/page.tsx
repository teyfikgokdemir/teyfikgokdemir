...
type Ready={ok?:boolean;ready?:boolean;database?:string;schema?:string;service?:string;time?:string;error?:string};
...
export default function StatusPage(){
  ...
  async function load(){
    ...
    try{
      ...
      setReady(readyBody);
      setCapabilities(capRes.ok?capBody:null);
      if(!readyRes.ok){
        const reason=readyBody.schema==='not_ready'?'Veritabanı erişilebilir ancak schema migration hazır değil.':readyBody.database==='unreachable'?'Veritabanına erişilemiyor.':'API readiness kontrolü başarısız.';
        setError(readyBody.error||reason);
        return;
      }
      if(!capRes.ok){
        setError(capBody.error||'Capability bilgisi alınamadı.');
        return;
      }
    }catch(e){
      if(sequence!==loadSequence.current)return;
  ...
  const schemaOk=ready?.schema==='ready';
  ...
  return <main className="statusScreen"><section className="statusShell">
    ...
    <section className="statusGrid" aria-busy={loading}>
      ...
      <article className={schemaOk?'ok':'warn'}><span>Schema / Migration</span><strong>{loading?'KONTROL…':schemaOk?'HAZIR':ready?.schema==='not_ready'?'HAZIR DEĞİL':'BİLİNMİYOR'}</strong><small>{ready?.schema||'—'}</small></article>
