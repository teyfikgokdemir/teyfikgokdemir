...
export default function Home() {
  ...
  return <main className="shell">
    ...
    <section className="content">
      ...
      {section==='ads' && <AdsView audit={audit} integrations={integrations} />}
...
function AdsView({audit,integrations}:{audit:Audit|null;integrations:IntegrationRow[]}) {
  const providers=['google_ads','meta_ads','tiktok_ads'];
  return <div className="moduleStack"><section className="moduleHero"><div><p className="eyebrow">Ads Readiness</p><h2>Reklam bütçesinden önce altyapıyı doğrula.</h2><p>Ölçümleme, dönüşüm sinyalleri ve hesap bağlantıları tek görünümde.</p></div><Score label="Ads Ready" value={audit?.scores.adsReadiness||0}/></section><section className="integrationGrid">{providers.map(provider=>{const found=integrations.find(i=>i.provider===provider);const label=provider==='google_ads'?'Google Ads':provider==='meta_ads'?'Meta Ads':'TikTok Ads';return <article key={provider} className={found?.status==='connected'?'connected':''}><div className="integrationIcon">{label.slice(0,1)}</div><div><strong>{label}</strong><span>{found?.account_label||'Hesap bağlanmadı'}</span></div><em>{found?.status==='connected'?'BAĞLI':'BEKLİYOR'}</em></article>})}</section><section className="readinessChecklist"><div><span>Audit skoru</span><b>{audit?.overallScore??'—'}</b></div><div><span>Ads readiness</span><b>{audit?.scores.adsReadiness??'—'}</b></div><div><span>Bağlı hesap</span><b>{integrations.filter(i=>i.status==='connected').length}/3</b></div></section><div className="moduleFoot">Reklam connector'ları read-only mimaride tutuluyor. Hesap anahtarları eklenmeden dış platformlarda işlem yapılmaz.</div></div>;
}
