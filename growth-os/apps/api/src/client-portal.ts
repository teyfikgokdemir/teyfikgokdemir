...
function ensureClientPortalSchema(){
...
async function resolveClientUser(workspaceId:string,clientId:string,email:string){
  ...
  const {rows}=await pool.query(`
    ...
    limit 1`,[workspaceId,clientId,email]);
...
clientPortalRouter.get('/:clientId/portal',async(req,res)=>{
  try{
    ...
    if(projectIds.length===0)return res.json({client,user,branding:branding.rows[0]||null,projects:[],summary:{},recommendations:[],results:[],decisions:[]});
    ...
    res.json({
      ...
      summary:{projectCount:projects.rows.length,latestAudits:auditSummary.rows,pendingRecommendations:recommendations.rows.filter((r:{status:string})=>r.status==='proposed').length,verifiedResults:results.rows.length,metrics30d:{spend,revenue,grossProfit:Number(m.gross_profit||0),conversions:Number(m.conversions||0),roas:spend>0?revenue/spend:null}},
...
clientPortalRouter.post('/:clientId/portal/recommendations/:recommendationId/decision',async(req,res)=>{
  ...
  try{
    ...
    const rec=await pool.query(`select r.id,r.project_id,r.status from recommendations r join projects p on p.id=r.project_id where r.id=$1 and p.workspace_id=$2 and p.client_id=$3 and r.status in ('proposed','approved')`,[req.params.recommendationId,workspaceId,clientId]);
    ...
    const {rows}=await pool.query(`
      ...
      returning *`,[workspaceId,clientId,rec.rows[0].project_id,req.params.recommendationId,email,parsed.data.decision,parsed.data.note||null]);
...
clientPortalRouter.get('/:clientId/portal-users',async(req,res)=>{
  try{
    ...
    const {rows}=await pool.query('select id,email,display_name,role,status,permissions,created_at,updated_at from client_portal_users where workspace_id=$1 and client_id=$2 order by created_at',[actor.workspaceId,req.params.clientId]);
    res.json(rows);
  }catch(error){res.status(errorStatus(error)).json({error:errorMessage(error)})}
});
...
clientPortalRouter.post('/:clientId/portal-users',async(req,res)=>{
  ...
  try{
    ...
    const {rows}=await pool.query(`
      ...
      returning id,email,display_name,role,status,permissions,created_at,updated_at`,[actor.workspaceId,req.params.clientId,d.email,d.displayName||null,d.role]);
